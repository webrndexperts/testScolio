...
<div class="card">
    <h5 class="card-header">Edit Attributes</h5>
    <div class="card-body">
      <form method="post" action="{{route('product-attributes.update',$category->id)}}">
        @csrf 
        @method('PATCH')
      ...
      <div class="form-group {{(($category->is_parent==1) ? 'd-none' : '')}}" id='parent_cat_div'>
          ...
          <input id="inputTitle" type="text" name="sku"  value="{{$category->sku}}" class="form-control">
          @error('sku')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
