...
@section('main-content')
 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary float-left">Contact Form List</h6>
    </div>
	...
	<div class="custom-lang">
		...
		@php
		 ...
		 $menu_model = App\Models\ContactForm::select('id','lang')->where('lang', $language_slug)->first();
		 $total_count_menu = App\Models\ContactForm::select('id','lang')->where('lang', $language_slug)->count();
			$menu_id = '';
			$menu_lang = '';
			if ($menu_model) {
        ...
        $urlWithParam = route('contact-form.index', ['lang' => $menu_lang]);
		@endphp
             ...
             // dd($post_parents_info);
              ...
              @endphp
                <tr>
                    <td>{{$i}}</td>
                    <td>{{$page->name}}</td>
					<td>{{$page->email_address}}</td>
					<td>{{$page->form_type}}</td>
                    <td>{{$page->lang}}</td>                
                    <td>
                    ...
                    <td>
                        <a href="{{route('contact-form.show',$page->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="View" data-placement="bottom"><i class="fas fa-eye"></i></a>
                    <form method="POST" action="{{route('contact-form.destroy',[$page->id])}}">
                      @csrf 
                      @method('delete')
