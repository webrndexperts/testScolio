...
<h5 class="card-header">Order      
 ...
 <a href="{{route('order.index')}}" class=" btn btn-sm btn-primary shadow-sm float-right"> Back</a>
  ...
  <div class="card-body">
			    ...
			    @php				
				   if ($order) {
						...
						foreach ($orderProducts as $orderItemInfo) {
							...
							$productPrice = $order['lang'] == 'en_MY' ? $orderItemInfo->product->malaysian_price : $orderItemInfo->product->price;
    ...
    <td>
    <?php
    // Check if the 'grouped_product_attributes' is an array or JSON string
    if (is_string($order->grouped_product_attributes)) {
            ...
            foreach ($data as $item) {
                ...
                if (is_array($item)) {
                    foreach ($item as $key => $value) {
                        if ($value !== null) {
                            // Display image if key is 'Image'
                            if ($key === "CustomizedImgage") {
                                echo "<p><b>Uploadeded File</b> <img src='$value' alt='' style='height:50px;'></p>";
                            } else {
 ...
 <a href="{{route('order.index')}}" class=" btn btn-sm btn-primary shadow-sm float-right"> Back</a>
  ...
  <div class="card-body">
    ...
    @php
        ...
        $formatPrice = function($amount) use ($currency, $currencySymbol) {
              ...
              <table class="table">
                @php                
                   if ($order) {
                     ...
                     <tr class="">
                        <td>Product Details :</td>
                         <td>
                         <b>Name-></b>  {{$productTitle ?? ''}}, 
                         <b>Price-></b>  {{ $formatPrice($productPrice ?? 0) }},
                ...
                @endphp             
                    ...
                    <tr class="">
                        ...
                        <td>
                            @php
                                $attributes = json_decode($order->grouped_product_attributes, true) ?? [];
                                foreach($attributes as $item) {
                                    if(is_array($item)) {
                                        foreach($item as $key => $value) {
                                            if($value !== null) {
                                                if($key === "CustomizedImgage") {
                                                    echo "<p><b>Uploaded File</b> <img src='$value' alt='' style='height:50px;'></p>";
                                                } else {
                                                    echo "<p><b>$key:</b> $value<br></p>";
                                                }
                                            }
                    ...
                    <tr>
                      <td>Coupon Discount</td>
                      <td> : {{ $formatPrice($order->coupon) }}</td>
                    </tr>
                    @endif
                     <tr>
                        <td>GST Tax</td>
                        <td> : {{ $formatPrice($order->gst_tax) }}</td>
                    </tr>
          ...
          <div class="col-lg-6 col-lx-4">
            <div class="shipping-info">
              ...
              <table class="table">
                    ...
                    <tr class="">
                        <td>Shipping Price:</td>
                        <td> : {{ $formatPrice($order->shipping_price) }}</td>
                    </tr>
