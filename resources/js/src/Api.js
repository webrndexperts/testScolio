...
export const checkForgetToken = async (token, email) => {
    try {
        const check = await axios.get(
            `${API}reset/check-token/${token}/${email}`
        );
...
export const getOpeningHours = async (lang) => {
    try {
        const openingHours = await axios.get(
            `${API}openinghourswidget/filter/${lang}`
        );
...
export const getRazorPayOrder = async (amount, currency) => {
    try {
...
export const getOnlineBookingUrl = async () => {
    try {
        const response = await axios.get(`${API}online-booking-url`);
        return response.data;
    } catch (error) {
        toast.error(error?.response?.data.message);
    }
