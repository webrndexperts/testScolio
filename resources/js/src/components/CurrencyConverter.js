    ...
    useEffect(() => {
        if (!loading && exchangeRates[targetCurrency]) {
            ...
            if (typeof currency === "string" && currency.includes("-")) {
                const [min, max] = currency.split("-").map(Number);
                if (!isNaN(min) && !isNaN(max)) {
                    setConvertedAmount(`${(min * conversionRate).toFixed(2)} - ${(max * conversionRate).toFixed(2)}`);
                } else {
                    setConvertedAmount(null); // fallback in case of bad format
                }
            } else {
                ...
                if (!isNaN(value)) {
                    ...
                    setConvertedAmount(null); // fallback
                }
            }
        }
    ...
    return (
        <div>
            {!loading && targetCurrency && convertedAmount ? ( 
                <p className="approx-price"> (Approx {getCurrencyInfo(currentLanguage).symbol} {convertedAmount} {targetCurrency}) </p>
            ) : (
