...
const Total = (props) => {
	...
	const storeAbandonedCart = async (cartData) => {
			...
			try {
			const response = await axios.post(`https://rndexperts.in/backend-laravel/api/v1/store-abandon-cart`, cartData);
		...
		useEffect(() => {
			if (!isCartDataStored && cart && cart.length > 0 && authData && authData.id) {
				setIsCartDataStored(false);
	...
	const storeAbandonedCartData = async () => {
	if (cart && cart.length > 0) {
		...
		// Call your storeAbandonedCart function here
		const storeResponse = await storeAbandonedCart(cartData);
		setIsCartDataStored(true); // Set flag to true after storing

		console.log('Abandoned cart data stored successfully', storeResponse);
		} catch (error) {
		console.error('Error storing abandoned cart data:', error);
		}
	}
	};
