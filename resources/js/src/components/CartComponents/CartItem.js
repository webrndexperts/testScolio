...
const CartItem = (props) => {
  ...
  // Get currency from localStorage, default to SGD
  const currency = JSON.parse(localStorage.getItem("currency")) || { currency: "SGD", symbol: "$" };

  // Save cart to localStorage (consider if this is necessary, as Redux should manage state)
  useEffect(() => {
    localStorage.setItem("cart", JSON.stringify(cart));
  }, [cart]);

  // Format price, handle undefined or invalid cases
  const formattedPrice = price !== undefined && !isNaN(price) ? parseFloat(price).toFixed(2) : "0.00";
  ...
  return (
    <div className="custom-cart-mainPage">
      ...
      <div className="custom-product-title">
        ...
        {attriuteImg && (
          <React.Fragment>
            <div className="variation-UploadedFile">{t("CART12.Uploaded File")}:</div>
            <div className="variation-UploadedFile">
              <p>
                <img
                  decoding="async"
