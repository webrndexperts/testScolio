...
const Total = (props) => {
  ...
  const taxRates = async () => {
    ...
    try {
      ...
      console.log("taxRates error:", error);
    }
  };
  ...
  const ChangeAddressSubmit = async (data) => {
    if (isApiCalling.current || !isDataValid()) {
      setLoader(false);
      return;
    }
    ...
    try {
      ...
      if (!dimensions) {
        setLoader(false);
        errorToast(t("CART12.dimension_calculation_failed"));
        isApiCalling.current = false;
        return;
      }

      data["contact_name"] = authData && authData.id ? authData.name : "Customer";
      ...
      data["items_description"] = cart[0]?.title || "Health Product";
      ...
      data["items_sku"] = cart[0]?.sku || "";
      data["items_hs_code"] = cart[0]?.hs_code || "";
      data["items_origin_country_alpha2"] = "SG";

      const shippingData = await axios.post(`${API}shipping-rates`, data);
      setLoader(false);

      if (!shippingData.data.success) {
        errorToast(shippingData.data.message || "Failed to retrieve shipping rates");
        isApiCalling.current = false;
        return;
      }
      ...
      setShippigCharges(0);
      console.error("Shipping details error:", error.response?.data?.message || error.message);
      // errorToast(error.response?.data?.message.join(', ') || error.message || t("checkOut.select_shipping"));
      errorToast(error.response?.data?.message || error.message || t("checkOut.select_shipping"));
      isApiCalling.current = false;
    }
  };
  ...
  const errorToast = (message) => {
    if (!toast.isActive(message)) {
      toast.error(message, { toastId: message, className: "full-red-alert", autoClose: 5000 });
    }
  };

  /*************************************************************
   * Abandoned cart functioning
   *************************************************************/
  ...
  useEffect(() => {
    if (authData && authData.id && cart?.length > 0) {
      storeAbandonedCartData();
    }
  }, [cart, totalPrice, selectedOption, shippigCharges]);
  ...
  useEffect(() => {
    if (shippingDetail && counterShip === 0 && cart?.length > 0) {
  ...
  }, [shippingDetail, searchcountries, cart]);

  useEffect(() => {
    if (states?.length && shippingState && stateFirst === 0) {
      let obj = states.find(o => o.name === shippingDetail?.shipping_information.state);
      setSelectedState(obj);
      setStateFirst(1);
    }
  }, [states, shippingState, shippingDetail]);
