...
const Article = () => {
    ...
    const triggerApi = () => {
        if (parentId) {
            fetch(`${APIUrl}post_all_slug/${parentId}`)
    ...
    const navigateToArticle = () => {
        if (slugsData && slugsData.length) {
            ...
            const item = slugsData[currentLanguage];
            if (typeof item != "undefined" && item) {
                navigate(`${urlLanguage}/articles/${item}`);
            }
        } else {
    ...
    useEffect(() => {
        const getPost = async () => {
            try {
              setIsLoading(true);
                const response = await fetch(APIUrl + `post_with_all_slug`, {
                    method: "POST",
                    headers: { 
                ...
                setData(json?.data?.article);
                setSlugsData(json?.data?.slugs);

                let _metaProps = {
                  tags: (json && json.data?.article.seo_meta_tag) ? json.data?.article.seo_meta_tag : '',
                  title: (json && json.data?.article.seo_meta_title) ? json.data?.article.seo_meta_title : '',
                  description: (json && json.data?.article.seo_meta_description) ? json.data?.article.seo_meta_description : '',
                }
      
                setMetaProps(_metaProps);

                const correctSlug = json.data.slugs[currentLanguage];

                if (correctSlug && correctSlug !== postId) {
                    if (currentLanguage !== "en_US") {
                        navigate(`/${currentLanguage}/articles/${correctSlug}`);
                    } else {
                        navigate(`/articles/${correctSlug}`);
                    }
                }
        ...
        getPost();
    }, [currentLanguage, API, postId, lang, navigate]);
                ...
                <header className="article-titel">
                    ...
                    <h5 className="social-share-post">
                        <div className="addtoany_shortcode">
                            ...
                            <a
                                href="https://twitter.com/i/flow/login?redirect_after_login=%2Fscoliolife"
                            ...
                            <a
                                href="https://www.addtoany.com/add_to/whatsapp?linkurl=https%3A%2F%2Fscoliolife.com%2Fthe-role-of-muscles-in-stabilization-of-the-spine-in-scoliosis-rehabilitation%2F&linkname=Muscles%20in%20Stabilization%20of%20the%20Spine%20In%20Scoliosis%20Rehabilitation&linknote=In%20scoliosis%20specific%20exercise%20there%20are%202%20different%20camps%20of%20thought%20when%20it%20comes%20to%20strengthening%20weakened%20muscles%20to%20help%20stabilize%20the%20spine."
                                title="WhatsUp"
                                target="blank"
                            >
