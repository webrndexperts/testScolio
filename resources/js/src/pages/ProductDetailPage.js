...
// import { getCurrentCountry, uploadFileApi } from '../Api';
...
//   //           setProductPrice(data.indonesian_price != null ? parseInt(data.indonesian_price) : parseInt(data.price));
//   //         } else if (country?.country == 'MY') {
//   //           setProductPrice(data.malaysian_price != null ? parseInt(data.malaysian_price) : parseInt(data.price));
//   //         } else {
...
//       try {
//         scrollToTop();
//         const params = authData?.id ? `?user=${authData.id}` : '';
//         setPriceLoad(true)
//         const response = await fetch(`${API}products/${slug}/${currentLanguage}${params}`);
//         if (!response.ok) {
//           throw new Error(`HTTP error! Status: ${response.status}`);
//         }
...
//         const sum = numericData.reduce((acc, obj) => acc + obj.value, 0);
//         const average = sum / data.product_review.length;
//         setAverageRating(average.toFixed(2)); // Fix decimal places properly
  
//         // Set product details
//         setProductDetail(data);
//         setUpdateAproxPrice((prev) => !prev);
  
//         // Set meta properties
//         setMetaProps({
//           tags: data?.seo_meta_tag || '',
...
//         // Get and set the price based on country
//         try {
...
//          if (countryCode === 'en_MY' && data.malaysian_price) {
//             price = parseInt(data.malaysian_price);
...
//           setProductPrice(parseInt(data.price) || 0);
//           setPriceLoad(false)
//         }
//         setPriceLoad(false)
//       } catch (error) {
...
//                         {!priceLoad ? <ProductPriceView price={productPrice} currentLanguage={currentLanguage}/> : <span> Loading ... </span>}
...
//                 // Determine the correct price based on the country
//                 let price = parseFloat(userData.price).toFixed(2); // Default to main price (SGD)
                
//                 if (storedCurrency.currency === "MYR" && userData.malaysian_price) {
//                   price = parseFloat(userData.malaysian_price).toFixed(2);
//                 } else {
...
//                         <p className="price">{storedCurrency.symbol} {price} {storedCurrency.currency}</p>
...
// const ProductPriceView = ({ price , currentLanguage}) => {
//   const currency = JSON.parse(localStorage.getItem('currency'))
//   const [priceVal, setPriceVal] = useState(`$0.00 ${currency.currency}`);
//   const [isLoading, setIsLoading] = useState(true);

//   // Format price values
//   const createValue = (val) => `${currency.symbol} ${parseFloat(val).toFixed(2)} ${currency.currency}`;

//   // Handle price ranges
//   const breakCurrencyRange = () => {
//     if (typeof price === "string" && price.includes("-")) {
//       const [min, max] = price.split("-");
//       setPriceVal(`${createValue(min)} – ${createValue(max)}`);
//     } else {
//       setPriceVal(createValue(price));
//     }
//   };
...
//       setIsLoading(false);
//     }
//   }, [price, currency, currentLanguage]);

//   if (isLoading) {
//     return <p className="lead">Loading...</p>;
//   }
//   return <p className="lead">{priceVal}</p>;
// };
...
const ProductDetailPage = () => {
  ...
  const checkVariationsData = async () => {
    ...
    if (checkVariable.includes("Language")) {
      if (languages) {
        variationsDet = { ...variationsDet, attriuteLang: languages };
      } else {
        setShowAlert(true);
      }
    } else {
    ...
    if (checkVariable.includes("Customized Report")) {
      if (customized) {
        variationsDet = { ...variationsDet, attriuteCustomized: customized };
        if (customized == "Yes") {
          if (!image) {
            setShowAlert(false);
            setimage(true);
    ...
    if (checkVariable.includes("Tool")) {
      if (tool) {
        variationsDet = { ...variationsDet, attriuteSize: tool };
      } else {
        setShowAlert(true);
      }
    } else {
    ...
    if (
      ...
      if (selectedSize) {
        variationsDet = { ...variationsDet, attriuteTool: selectedSize };
      } else {
        setShowAlert(true);
      }
    } else {
    ...
    if (checkVariable.includes("Consultation Type")) {
      if (consultationSize) {
        variationsDet = { ...variationsDet, attriuteConsultant: consultationSize };
      } else {
        setShowAlert(true);
      }
    } else {
    ...
    if (checkVariable.includes("Gender")) {
      if (gender) {
        variationsDet = { ...variationsDet, attriuteGender: gender };
      } else {
        setShowAlert(true);
      }
    } else {
    ...
    if (checkVariable.includes("Height")) {
      if (height) {
        variationsDet = { ...variationsDet, attriuteHeight: height };
      } else {
        setShowAlert(true);
      }
    } else {
    ...
    if (checkVariable.includes("Weight")) {
      if (weight) {
        variationsDet = { ...variationsDet, attriuteWeight: weight };
      } else {
        setShowAlert(true);
      }
    } else {
  ...
  const checkVariableAndAddToCart = async (type = 'cart') => {
    if (selectedSize && consultationSize && languages && customized && tool && image && gender && height && weight) {
      ...
      let itemDataWithLanguage = {
        ...
        attriuteImg: image == true ? "" : image,
        price: finalPrice,
        originalPrice: parseInt(productDetail?.price) || 0, // Store SGD price
        malaysianPrice: parseInt(productDetail?.malaysian_price) || parseInt(productDetail?.price) || 0 // Store MYR price
      };
      if (type == 'direct') {
        dispatch(addToDirectCart(itemDataWithLanguage));
        navigate(`${urlLanguage}/checkout`);
  ...
  const onBuyNowClick = async () => {
    ...
    if (isVariable) {
      ...
      dispatch(addToDirectCart({
        ...itemData,
        originalPrice: parseInt(productDetail?.price) || 0,
        malaysianPrice: parseInt(productDetail?.malaysian_price) || parseInt(productDetail?.price) || 0
      }));
      navigate(`${urlLanguage}/checkout`);
    }
  };
  ...
  const handleSelectChange = (event) => {
    if (event.target.value === "customised_streaming") {
      ...
      setSelectedType(false);
      setProductPrice(currentLanguage === 'en_MY' ? 
        (parseInt(productDetail?.malaysian_price) || parseInt(productDetail?.price) || 0) : 
  ...
  const UploadAddToCart = async (event) => {
    ...
    let itemDataWithLanguage = {
      ...itemData,
      attriuteImg: event?.CustomizedImgage?.[0] ? URL.createObjectURL(event.CustomizedImgage[0]) : "",
      CustomizedImgage: _image,
    };
    dispatch(addToCart(itemDataWithLanguage));
    navigate(`${urlLanguage}/cart`);
  ...
  useEffect(() => {
    try {
      axios.post('https://sladmin.scoliolife.com/api/v1/get-aws-bucket-order', {
        user_id: (authData && authData.id) ? authData.id : null
      })
      .then((response) => {
        const data = response.data;
  ...
  return (
    <>
                  ...
                  <div className="product-text">
                    ...
                    {(productDetail?.price || productDetail?.price > 0) && (
                      <Fragment>
                        <div className="rating-wishlist-div">
                          <div className="product-star">
                            <p className="star"><Rating stars={averageRating} /></p>
                            <p>({productDetail?.product_review?.length}{t('product.customer_reviews')})</p>
                          </div>
                          <WishlistIcon product={productDetail} {...wishProps} />
                        </div>
                        {!priceLoad ? <ProductPriceView price={productPrice} currentLanguage={currentLanguage}/> : <span> Loading ... </span>}
                    ...
                    {(productDetail?.price || productDetail?.price > 0) && (
                      <div className="product-cart">
                        <span className="cart-minus" onClick={() => handleMinusQuantity()}><HiMinus /></span>
                        <h4>{quantity}</h4>
                        <span className="cart-plus" onClick={() => handlePlusQuantity()}><BsPlusLg /></span>
                      </div>
                    )}
                    <Item
                      key={productDetail?.id}
                    ...
                    <div>
                      ...
                      <a href="https://x.com/i/flow/login?redirect_after_login=%2Fscoliolife" title="Follow Us on X" target="blank">
          ...
          <div className="tabs-product">
            <div className="tab-content">
              ...
              <div className="product-listing">
                <div className="product-listing-description">
                  <div className="product-description">
                    ...
                    {!read ? (
                      <div>
                        <p dangerouslySetInnerHTML={{ __html: productDetail?.description }}></p>
                        <p className="read_more" onClick={() => setRead((prevOpen) => !prevOpen)}>
                          {t("Patients.read_more")}
                        </p>
                      </div>
                    ) : (
                      <div>
                        <p dangerouslySetInnerHTML={{ __html: productDetail?.summary }}></p>
                        <p className="read_more" onClick={() => setRead((prevOpen) => !prevOpen)}>
                          {t("Patients.read_less")}
                        </p>
                      </div>
                  ...
                  <div className="product-table">
                    <table className="table">
                      <tbody className="table-section">
                        {initialPosts?.map((item, index) => (
                          <tr className="product-video-tr" key={item.id} onClick={() => toggleModal(item, index, productDetail?.aws3_bucket_product?.length)}>
                            <td className="product-video-index">{index + 1}</td>
                            <td className="product-video-title">{item?.video_name}</td>
                            <td className="product-video-duration">{item?.video_duration}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    {isCompleted ? (
                      <div className="show_more_td_empty"><strong>{t("product-detail.episodes")}</strong></div>
                    ) : (
                      <div className="show_more_td_empty" onClick={() => loadMore()}>
                        <strong>+{productDetail?.aws3_bucket_product?.length - initialPosts.length} {t("product-detail.more episodes")}</strong>
                      </div>
                    )}
                  </div>
                </div>
                <div className="product-listing-sidebar-main">
                  <div className="product-listing-sidebar">
                    <div className="sidebar_images"><img src={Image} alt="" /></div>
                    {(!CheckLogin || (awsData && awsData?.data?.length === 0)) && (
                      <p className="Confused">{t("product-detail.Confused")}</p>
                    )}
                    {(!CheckLogin || (awsData && awsData?.data?.length === 0)) && (
                      <form id="ProductTypeForm" onSubmit={handleSubmit(UploadAddToCart)}>
                        <label htmlFor="product_purchase_type">{t("product-detail.Choose Product")}</label>
                        <select
                          name="product_purchase_type"
                          id="product_purchase_type"
                          {...register("productOption", { required: true })}
                          onChange={handleSelectChange}
                        >
                          <option value="" className="enuiry_meta">{t("product-detail.Select an option")}</option>
                          <option value="stream_plus_download" className="enuiry_meta">{t("product-detail.Stream Scoliosis Exercises")}</option>
                          <option value="customised_streaming" className="enuiry_meta">{t("product-detail.Stream")}</option>
                        </select>
                        {errors.productOption && <p className="validations">{t("product-detail.option")}</p>}
                        <input name="product_custom_streaming" type="hidden" value="1" />
                        {selectedType ? (
                          <>
                            <div className="wau_wrapper_div">
                              <label htmlFor="wau_file_addon">{t("product-detail.Upload an image")}</label>
                              <input
                                type="file"
                                name="wau_file_addon"
                                id="wau_file_addon"
                                ...
                                {...register("CustomizedImgage")}
                              />
                              {errors.CustomizedImgage && <p className="validations">{t("product-detail.Upload")}</p>}
                            </div>
                            <span id="vaild_err"></span>
                          </>
                        ) : null}
                        <div className="ul_price_new">
                          ...
                          >
                            <div className="pr_cont">
                              <div className="svd">
                                <img src="https://sladmin.scoliolife.com/uploads/2022/10/thumb_Streaming.png" alt="" />
                              </div>
                              <div className="price_gr_p">
                                <div className="price_gr">${parseFloat(productPrice).toFixed(2)} SGD</div>
                                <span className="stream-download">{t("product-detail.Customized")}</span>
                              </div>
                            </div>
                          </button>
                        </div>
      ...
      {productDetail?.rel_prods?.length > 0 ? (
        <div className="related-product">
          <div className="row">
            <h2>{t('product.related')}</h2>
            {productDetail?.rel_prods?.slice(0, 4).map((userData) => {
              let storedCurrency = JSON.parse(localStorage.getItem("currency")) || { currency: "SGD", symbol: "$" };
              let price = parseFloat(userData.price).toFixed(2);
              if (storedCurrency.currency === "MYR" && userData.malaysian_price) {
                price = parseFloat(userData.malaysian_price).toFixed(2);
              } else {
                price = parseFloat(userData.price).toFixed(2);
                storedCurrency = { currency: "SGD", symbol: "$" };
              }
              return (
                <div key={userData.id} className="col-sm-3">
                  <div className="shop-treatments">
                    ...
                    <div className="home__container">
                      <div className="home__row">
                        <div>
                          <button className="btn btn-primary w-100" onClick={() => onAddCartClick(userData.slug)}>
                            {t("product_dropdown.add_to_cart")}
                          </button>
                        </div>
                      </div>
      ...
      {(slug == 'skype-zoom-consultation') ? <FranForm /> : null}
      <Modal isOpen={modal} onRequestClose={toggleModal} contentLabel="Example Modal">
        <div className="product-model">
          <div className="product-model-heading">
            <h5 className="product-model-heading">{modalData?.data?.video_name}</h5>
            <span className="close-btn" onClick={() => setModal((modal) => !modal)}>
              <i className="fa fa-times" aria-hidden="true"></i>
            </span>
          </div>
          {(modalData?.allowed) ? (
            ...
            <div className="product-model-image-description">
              <div className="product-model-image">
                <div className="product-image"><img src={modalData?.data?.image_link} alt="" /></div>
                <span className="product-model-duration">{t("product-detail.Duration")} {modalData?.data?.video_duration}</span>
              </div>
              <div className="product-model-description">
                <h4 className="product-model-title">{modalData?.data?.video_name}</h4>
                <p className="product-model-p">{t("product-detail.Episode")} {modalData?.resialNo} of {modalData?.length}</p>
                <div className="product-model-buy">
                  <p>{t("product-detail.available")}</p>
                  <div className="product_price_dv">
                    <div className="iris_ic is--16">
...
const ProductPriceView = ({ price, currentLanguage }) => {
  ...
  const [priceVal, setPriceVal] = useState(`${currency.symbol}0.00 ${currency.currency}`);
  const [isLoading, setIsLoading] = useState(true);
